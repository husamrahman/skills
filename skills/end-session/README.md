...
Wrap up a work session by writing back what it taught you. This skill captures the session as a structured note in your markdown vault, in clear named sections - summary, changes, decisions, gotchas, lessons learned, next steps, handoff - then commits, pushes, and opens a PR.
...
2. Writes a structured session log to `<vault>/sessions/` with these sections: **Session summary, Changes, Decisions, Gotchas, Lessons learned, Next steps, Handoff**.
3. Works out the gotchas and lessons itself - the non-obvious ones only - each lesson with a concrete "how you'll know it stuck" check.
4. Promotes reusable gotchas and lessons into the project note (`projects/`) or, if global, into `knowledge/`.
...
agent > Wrote sessions/acme-api-2026-08-16-142230.md
        ...
        Lesson learned: rate-limit tests need a no-auth request case.
          How you'll know: a request with no auth header is limited, not exempted.
